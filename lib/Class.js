    ...
    implements(...interfaces) {
        ...
        interfaces.forEach(implementedInterface => {
            ...
            if (unimplementedMethods.length > 0) {
                throw new InterfaceError(`Methods [${unimplementedMethods.map(method => (method.isStatic ? 'static ' : '') + method.name).join(', ')}] are not defined on class ${this.class.prototype.constructor.name} (class ${this.class.prototype.constructor.name} implement interface ${implementedInterface.constructor.name})`);
            }
        });
    }
}
