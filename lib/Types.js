...
export class TypeJSType {
    ...
    declare(name) {
        if (name && ScriptTypeJS.mode == 'dev') {
            ScriptTypeJS.types[name] = this;
        }
    }
}
...
export class Type extends TypeJSType {
    constructor(arg, name) {
        super(arg, name);
        if (ScriptTypeJS.mode !== 'dev' || typeof arg !== 'object') return undefined;
    }
...
export function T(paramObject, name) {
    return new Type(paramObject, name);
}
