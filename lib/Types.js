...
export class TypeJSType {
    ...
    static declare(name, enumeration) {
        if (name && TypeJS.mode == 'dev') {
            TypeJS.types[name] = enumeration;
        }
    }
}
...
export class TypeDeclaration extends TypeJSType {
    constructor(arg, name) {
...
export function T(paramObject, name) {
    return new TypeDeclaration(paramObject, name);
}
